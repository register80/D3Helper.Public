...
namespace D3Helper.A_Handler.ParagonPointSpender
{
    class ParagonPointSpender
    {
        ...
        private static bool ResetNeeded(int[] PointsToSet, int Tab)
        {
            try
            {
                int[] AllTabPoints = new[]
                {
                    GetSpentPoints_OnGivenBonus(0), GetSpentPoints_OnGivenBonus(1), GetSpentPoints_OnGivenBonus(2),
                    GetSpentPoints_OnGivenBonus(3)
                };
                ...
                {
                    case 0:
                        MaxValues = A_Collection.Presets.ParagonPointSpender.MaxValues_Core;
                        break;

                    case 1:
                        MaxValues = A_Collection.Presets.ParagonPointSpender.MaxValues_Offense;
                        break;

                    case 2:
                        MaxValues = A_Collection.Presets.ParagonPointSpender.MaxValues_Defense;
                        break;

                    case 3:
                        MaxValues = A_Collection.Presets.ParagonPointSpender.MaxValues_Utility;
                        break;
                }
                ...
                {
                    case 0:
                        UnspentPoints = Get_UnspentPoints_Core();
                        break;

                    case 1:
                        UnspentPoints = Get_UnspentPoints_Offense();
                        break;

                    case 2:
                        UnspentPoints = Get_UnspentPoints_Defense();
                        break;

                    case 3:
                        UnspentPoints = Get_UnspentPoints_Utility();
                        break;
                }

                for (int i = 0; i < PointsToSet.Count(); i++)
                {
                    int value = PointsToSet[i];
                    int maxvalue = MaxValues[i];
                    int currentPoints = AllTabPoints[i];

                    if(value > 0 && currentPoints != value)
                        return true;

                    if (value == -1 && currentPoints != maxvalue && UnspentPoints > 0)
                        return true;

                    if (value == -1 && UnspentPoints > 0)
                        return true;
                }
