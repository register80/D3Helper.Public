...
namespace D3Helper.A_Tools
{
    class T_ACD
    {
        ...
        public static ActorCommonData get_BossInRange(List<ActorCommonData> MonstersInRange)
        {
            try
            {
                return MonstersInRange.FirstOrDefault(x => x.x0B8_MonsterQuality == Enigma.D3.Enums.MonsterQuality.Boss);
            }
            catch { return null; }
        }
        public static int count_AcdsBlinded(List<ActorCommonData> MonstersInRange)
        {
            int counter = 0;
            try
            {
                foreach (var monster in MonstersInRange.Where(x => x.x000_Id != -1).ToList())
                {
                    if (isBlinded(monster))
                    {
                        counter = counter + 1;
                    }
                }
        ...
        public static int get_PlayersInRange(int Range)
        {
            try
            {

                List<A_Collector.ACD> acdcontainer;
                lock(A_Collection.Environment.Actors.AllActors) acdcontainer = A_Collection.Environment.Actors.AllActors.ToList().Where(x => x._ACD.x184_ActorType == Enigma.D3.Enums.ActorType.Player).ToList();

                int counter = 0;

                foreach (var player in acdcontainer)
                {
